...
function getAyahCountMap() {
  const metadata = readJson(METADATA_PATH);
  const surahs = Array.isArray(metadata.surahs) ? metadata.surahs : [];
  const map = {};
  for (const surah of surahs) {
    if (Number.isInteger(surah.number) && Number.isInteger(surah.numberOfAyahs)) {
      map[surah.number] = surah.numberOfAyahs;
    }
  }
  return map;
}
...
function build() {
  const ayahCountMap = getAyahCountMap();
  const reciters = {
    ghamidi: {},
    muaiqly: {},
  };
  ...
  for (const reciter of RECITERS) {
    ...
    for (const fileName of files) {
      ...
      const firstAyah = payload.segments[0].ayah;
      const lastAyah = payload.segments[payload.segments.length - 1].ayah;
      if (firstAyah !== 1) {
        errors.push(`${reciter}/${fileName}: first ayah must be 1, got ${firstAyah}`);
        continue;
      }
      if (Number.isInteger(expectedAyahs) && lastAyah !== expectedAyahs) {
        errors.push(
          `${reciter}/${fileName}: ayah count mismatch, expected ${expectedAyahs}, got last ayah ${lastAyah}`
        );
        continue;
      }
