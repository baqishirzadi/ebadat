...
function DreamInterpreterWidgetInner({ onInputFocus }: DreamInterpreterWidgetProps) {
  ...
  const copy = useMemo(
    () => (state.preferences.appLanguage === 'pashto' ? DREAM_COPY.pashto : DREAM_COPY.dari),
    ...
    <RtlView style={styles.container}>
      <Pressable onPress={openFullChat} hitSlop={8} style={styles.titlePress}>
        <View style={styles.titleRow}>
          <RtlText align="center" wrap={false} style={styles.title}>{copy.title}</RtlText>
      ...
      {!isConfigured ? (
        <RtlText align="center" style={styles.configWarning}>سرویس تعبیر خواب هنوز پیکربندی نشده است.</RtlText>
      ) : null}
