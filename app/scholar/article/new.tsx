...
import { createArticle, updateArticle } from '@/utils/articleService';
...
export default function NewArticleScreen() {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      setSaving(true);
      await createArticle(scholarState.scholar.id, scholarState.scholar.fullName, {
        title: title.trim(),
        body: body.trim(),
        category,
        language,
  ...
  const handlePublish = async () => {
    ...
    try {
      setPublishing(true);
      const articleId = await createArticle(scholarState.scholar.id, scholarState.scholar.fullName, {
        title: title.trim(),
        body: body.trim(),
        category,
        language,
      ...
      if (notificationResult.success) {
        ...
        Alert.alert('موفق', 'مقاله منتشر شد (خطا در ارسال اعلان)');
      }
