    ...
    Thread {
      try {
        if (payload.type == "maintenance") {
          AdhanScheduleManager.handleMaintenanceAlarm(context.applicationContext)
        ...
        AdhanNotificationChannels.ensureCreated(context)
        ...
        AdhanAlarmScheduler.cancel(context, payload.id)
        AdhanScheduleManager.ensureScheduled(context.applicationContext, "alarm-fired-${payload.id}")
      } catch (error: Exception) {
        Log.e("AdhanAlarmReceiver", "Failed to handle alarm", error)
      } finally {
        pending.finish()
      }
    }.start()
