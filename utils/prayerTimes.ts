...
function getTimezoneOffsetHours(timeZone: string | undefined, date: Date): number {
  ...
  try {
    ...
    return (asUTC.getTime() - date.getTime()) / (60 * 60 * 1000);
  } catch {
...
function computeTime(
  ...
  const D = -sin(angle) - sin(latitude) * sin(declination);
...
function calculateWithAdhanFallback(date: Date, location: Location): PrayerTimes {
...
function validatePrayerTimesSanity(times: PrayerTimes): { ok: boolean; reason?: string } {
  const ordered = [times.fajr, times.sunrise, times.dhuhr, times.asr, times.maghrib, times.isha];

  if (ordered.some((d) => !(d instanceof Date) || isNaN(d.getTime()))) {
    return { ok: false, reason: 'invalid_dates' };
  }

  for (let i = 1; i < ordered.length; i++) {
    if (ordered[i].getTime() <= ordered[i - 1].getTime()) {
      return { ok: false, reason: 'unordered_times' };
    }
  }

  const uniqueMinutes = new Set(ordered.map((d) => Math.floor(d.getTime() / 60000)));
  if (uniqueMinutes.size < 4) {
    return { ok: false, reason: 'insufficient_unique_times' };
  }

  const daySpreadHours = (times.isha.getTime() - times.fajr.getTime()) / (60 * 60 * 1000);
  if (daySpreadHours <= 6) {
    return { ok: false, reason: 'spread_too_small' };
  }
...
export function calculatePrayerTimes(
  ...
  // Transit time (Dhuhr)
  const tzOffsetHours = getTimezoneOffsetHours(location.timezone, date);
  const dhuhrTime = 12 - equation + tzOffsetHours - longitude / 15;
  ...
  const calculatedTimes: PrayerTimes = {
  ...
  const sanity = validatePrayerTimesSanity(calculatedTimes);
  if (sanity.ok) {
  ...
  if (__DEV__) {
    console.warn(
      `[calculatePrayerTimes] Sanity fallback to adhan (${sanity.reason}) for lat=${location.latitude}, lon=${location.longitude}, date=${date.toISOString()}`
    );
  }
  return calculateWithAdhanFallback(date, location);
}
